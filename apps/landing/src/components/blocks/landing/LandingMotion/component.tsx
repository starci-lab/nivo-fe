...
export const LandingMotionArtworkDriftBase = (props: LandingMotionArtworkDriftProps) => {
  return <motion.div className={C.visualOverlay} initial={false} animate={props.isReducedMotion ? undefined : { y: [-4, 4, -4], rotate: [-0.2, 0.2, -0.2] }} transition={{ duration: 9, repeat: Infinity, ease: "easeInOut" }}>{props.children}</motion.div>;
};

/** Reveals the hero graph once, then lets CSS handle its ambient signal. */
export const LandingMotionResponsibilityGraphBase = (props: LandingMotionResponsibilityGraphProps) => {
  return <motion.div className={C.responsibilityGraph} initial={false} whileInView={props.isReducedMotion ? undefined : { opacity: [0.45, 1], scale: [0.96, 1] }} viewport={{ once: true, amount: 0.35 }} transition={{ duration: 0.72, delay: 0.18, ease: [0.22, 1, 0.36, 1] }}>{props.children}</motion.div>;
};

/** Reveals one operating-loop step in reading order. */
export const LandingMotionLoopStepBase = (props: LandingMotionLoopStepProps) => {
  return <motion.li style={{ "--loop-x": props.position } as CSSProperties} initial={false} whileInView={props.isReducedMotion ? undefined : { opacity: [0.5, 1], y: [8, 0] }} viewport={{ once: true, amount: 0.6 }} transition={{ duration: 0.42, delay: props.index * 0.065, ease: "easeOut" }}>{props.children}</motion.li>;
};

/** Illuminates the operating path in reading order as it enters view. */
export const LandingMotionLoopTrackBase = (props: LandingMotionLoopTrackProps) => {
  return <motion.div className={C.loopTrackShell} initial={false} whileInView={props.isReducedMotion ? undefined : { opacity: [0.7, 1] }} viewport={{ once: true, amount: 0.35 }} transition={{ duration: props.isReducedMotion ? 0 : 0.35 }}>{props.children}</motion.div>;
};

/** Moves responsibility across the three operating layers without scroll hijacking. */
export const LandingMotionRoleLayerBase = (props: LandingMotionRoleLayerProps) => {
  const direction = props.index === 1 ? 1 : -1;
  return <motion.article initial={false} whileInView={props.isReducedMotion ? undefined : { opacity: [0.5, 1], x: [direction * 28, 0] }} viewport={{ once: true, amount: 0.55 }} transition={{ duration: 0.56, delay: props.index * 0.11, ease: [0.22, 1, 0.36, 1] }}>{props.children}</motion.article>;
};

/** Reveals one responsibility instance with a restrained stagger. */
export const LandingMotionInstanceCardBase = (props: LandingMotionInstanceCardProps) => {
  return <motion.article initial={false} whileInView={props.isReducedMotion ? undefined : { opacity: [0.62, 1], x: [28, 0], rotate: [1.4, 0] }} viewport={{ once: true, amount: 0.45 }} transition={{ duration: 0.52, delay: props.index * 0.1, ease: [0.22, 1, 0.36, 1] }}>{props.children}</motion.article>;
};
