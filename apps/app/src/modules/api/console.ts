...
export type CatalogCategory = "ai_agent" | "digital_identity" | "launch_ai" | "migration" | "ready_made_site" | "site_from_template";
 ...
 */
export const myExpertSites = (): Promise<Result<ReadonlyArray<ExpertSiteRow>>> => graphql(`query MyExpertSites { myExpertSites { data ${EXPERT_SITE} message success error } }`);
 ...
 */
export const myAgentWorkspace = (): Promise<Result<ReadonlyArray<AgentWorkspaceRow>>> => graphql(`query MyAgentWorkspace { myAgentWorkspace { data ${AGENT_WORKSPACE} message success error } }`);
 ...
 */
export const myInstances = (): Promise<Result<ReadonlyArray<InstanceRow>>> => graphql(`query MyInstances { myInstances { data ${INSTANCE} message success error } }`);
 ...
 */
export const myDomains = (): Promise<Result<ReadonlyArray<DomainRow>>> => graphql(`query MyDomains { myDomains { data ${DOMAIN} message success error } }`);
 ...
 */
export const myWallet = (): Promise<Result<WalletRow>> => graphql(`query MyWallet { myWallet { data ${WALLET} message success error } }`);
 ...
 */
export const myWalletTransactions = (): Promise<Result<ReadonlyArray<WalletTransactionRow>>> => graphql(`query MyWalletTransactions { myWalletTransactions { data ${WALLET_TRANSACTION} message success error } }`);
 ...
 */
export const myInvoices = (): Promise<Result<ReadonlyArray<InvoiceRow>>> => graphql(`query MyInvoices { myInvoices { data ${INVOICE} message success error } }`);
 ...
 */
export const myCatalogOrders = (): Promise<Result<ReadonlyArray<CatalogOrderRow>>> => graphql(`query MyCatalogOrders { myCatalogOrders { data ${CATALOG_ORDER} message success error } }`);
...
export const myAgentosSolutionModules = (): Promise<Result<ReadonlyArray<AgentosSolutionModule>>> => graphql(`query MyAgentosSolutionModules {
...
export const revokeAgentWorkspaceAppLaunch = (launchId: string): Promise<Result<{
  readonly launchId: string;
...
export const resolveCoreApiCapabilityUrl = (capabilityUrl: string): string => {
  ...
  const apiUrl = new URL(configuredApiUrl, typeof window === "undefined" ? "http://localhost:3068" : window.location.origin);
  return new URL(capabilityUrl, `${apiUrl.origin}/`).toString();
};
...
export const uploadAgentosModuleAttachment = async (capability: Pick<AgentosModuleUploadCapability, "uploadUrl" | "uploadMethod">, mediaType: string, body: Blob): Promise<Result<boolean>> => {
  try {
    ...
    return response.ok ? {
      ...
      ok: false,
      reason: `upload refused with ${response.status}`,
      code: "UPLOAD_REFUSED"
    };
  } catch {
    return {
