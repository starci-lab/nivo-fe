...
export type SupportOperatorAction = { readonly id: string; readonly state: string }
/** Bounded cursor page returned by the workspace controller. */
export type SupportConnection<T> = { readonly nodes: ReadonlyArray<T>; readonly nextCursor: string | null }

type GraphqlEnvelope<T> = { readonly data?: T; readonly errors?: ReadonlyArray<{ readonly message?: string }> }
...
const request = async <T>(
...
): Promise<WorkspaceControlplaneResult<T>> => {
    ...
    try {
        ...
        if (!response.ok || envelope.data === undefined || (envelope.errors?.length ?? 0) > 0) {
            return { ok: false, code: response.status === 401 || response.status === 403 ? "WORKSPACE_CONTROLLER_REFUSED" : "WORKSPACE_CONTROLLER_FAILED" }
        }
...
export const supportCustomerConversations = async (
    hostname: string, workspaceId: string, accessToken: string, installationId: string,
): Promise<WorkspaceControlplaneResult<SupportConnection<SupportCustomerConversation>>> => {
    const result = await request<{ readonly supportCustomerConversations: SupportConnection<SupportCustomerConversation> }>(
        hostname, workspaceId, accessToken,
        `query SupportCustomerConversations($installationId: ID!) {
    ...
    return result.ok ? { ok: true, data: result.data.supportCustomerConversations } : result
}

/** Read the durable transcript for one authorized workspace conversation. */
export const supportCustomerMessages = async (
    hostname: string, workspaceId: string, accessToken: string, conversationId: string,
): Promise<WorkspaceControlplaneResult<SupportConnection<SupportCustomerMessage>>> => {
    const result = await request<{ readonly supportCustomerMessages: SupportConnection<SupportCustomerMessage> }>(
        hostname, workspaceId, accessToken,
        `query SupportCustomerMessages($conversationId: ID!) {
    ...
    return result.ok ? { ok: true, data: result.data.supportCustomerMessages } : result
}

/** Read evidence-backed Support workbench items for one module installation. */
export const supportTickets = async (
    hostname: string, workspaceId: string, accessToken: string, installationId: string,
): Promise<WorkspaceControlplaneResult<SupportConnection<SupportTicket>>> => {
    const result = await request<{ readonly supportTickets: SupportConnection<SupportTicket> }>(
        hostname, workspaceId, accessToken,
        `query SupportTickets($installationId: ID!) {
    ...
    return result.ok ? { ok: true, data: result.data.supportTickets } : result
}

/** Read source-cited important facts for one Support module installation. */
export const supportImportantFacts = async (
    hostname: string, workspaceId: string, accessToken: string, installationId: string,
): Promise<WorkspaceControlplaneResult<SupportConnection<SupportImportantFact>>> => {
    const result = await request<{ readonly supportImportantFacts: SupportConnection<SupportImportantFact> }>(
        hostname, workspaceId, accessToken,
        `query SupportImportantFacts($installationId: ID!) {
    ...
    return result.ok ? { ok: true, data: result.data.supportImportantFacts } : result
}

const mutate = async (
    hostname: string, workspaceId: string, accessToken: string, query: string, variables: Readonly<Record<string, unknown>>, field: string,
): Promise<WorkspaceControlplaneResult<SupportOperatorAction>> => {
...
export const approveSupportReply = (hostname: string, workspaceId: string, accessToken: string, decisionId: string) => mutate(
    hostname, workspaceId, accessToken,
    `mutation ApproveSupportReply($decisionId: ID!) { approveSupportReply(decisionId: $decisionId) { id state } }`,
    { decisionId }, "approveSupportReply",
)

/** Enable or release explicit human takeover for one customer conversation. */
export const setSupportTakeover = (hostname: string, workspaceId: string, accessToken: string, conversationId: string, takeover: boolean) => mutate(
    hostname, workspaceId, accessToken,
    `mutation SetSupportTakeover($conversationId: ID!, $takeover: Boolean!) { setSupportTakeover(conversationId: $conversationId, takeover: $takeover) { id state } }`,
    { conversationId, takeover }, "setSupportTakeover",
)

/** Reconcile one ambiguous provider timeout without unsafe blind retry. */
export const reconcileSupportDelivery = (hostname: string, workspaceId: string, accessToken: string, outboxId: string, delivered: boolean) => mutate(
    hostname, workspaceId, accessToken,
    `mutation ReconcileSupportDelivery($outboxId: ID!, $delivered: Boolean!) { reconcileSupportDelivery(outboxId: $outboxId, delivered: $delivered) { id state } }`,
    { outboxId, delivered }, "reconcileSupportDelivery",
)

/** Narrow transport hooks exposed only for focused endpoint-policy tests. */
export const workspaceControlplaneTesting = { endpointFor }
