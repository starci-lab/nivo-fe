...
export const SessionProvider = ({ children }: SessionProviderProps) => {
    ...
    const adopt = useCallback((payload: AuthPayload) => {
        ...
        setState({ status: "signed-in", accessToken: payload.accessToken })
    }, [])
    ...
    useEffect(() => {
        ...
        const restore = async () => {
            ...
            if (result.ok && !result.data.requiresTwoFactor && result.data.accessToken !== null) {
                ...
                setState({ status: "signed-in", accessToken: result.data.accessToken })
                return
            }
            /*
    ...
    const value = useMemo<Session>(() => ({ state, adopt, end }), [state, adopt, end])
