...
export const useOauthReturnExchange = () => {
    ...
    useEffect(() => {
        ...
        void exchange.trigger({ code, provider, state }).then(setAnswer)
    }, [exchange])

    return { answer, isMutating: exchange.isMutating }
}
