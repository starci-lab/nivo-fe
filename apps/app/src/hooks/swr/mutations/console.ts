...
export const useMutateSaveAgentosModuleIntegrationSecretSwr = (workspaceId: string, moduleId: string) => useNivoMutation(["agentos", "module-integration-save", workspaceId, moduleId], (input: SaveAgentosModuleIntegrationSecretCommand) => saveAgentosModuleIntegrationSecret({
  agentWorkspaceId: workspaceId,
  moduleId,
  ...input
}), {
...
export const useMutateRemoveAgentosModuleIntegrationSecretSwr = (workspaceId: string, moduleId: string) => useNivoMutation(["agentos", "module-integration-remove", workspaceId, moduleId], (providerKey: string) => removeAgentosModuleIntegrationSecret({
  agentWorkspaceId: workspaceId,
  moduleId,
  providerKey
}), {
...
export const useMutatePublishAgentosCustomModuleSwr = (workspaceId: string, moduleId: string) => useNivoMutation(["agentos", "custom-module-publish", workspaceId, moduleId], (input: PublishAgentosCustomModuleCommand) => publishAgentosCustomModule({
  agentWorkspaceId: workspaceId,
  moduleId,
  ...input
}), {
...
export const useMutateInstallAgentosSolutionModuleSwr = (workspaceId: string) => useNivoMutation(["agentos", "solution-module-install", workspaceId], (input: InstallAgentosSolutionModuleCommand) => installAgentosSolutionModule({
  agentWorkspaceId: workspaceId,
  ...input
}), {
  invalidates: [agentosModuleInstallationsQueryKey(workspaceId), agentWorkspaceControlCenterQueryKey(workspaceId)],
  shouldInvalidate: accepted
});

/** Issue one short-lived workspace application launch grant. */
export const useMutateIssueAgentWorkspaceAppLaunchSwr = (workspaceId: string) => useNivoMutation(["agentos", "workspace-app-launch-issue", workspaceId], () => issueAgentWorkspaceAppLaunch(workspaceId));

/** Revoke one exact workspace application launch grant. */
export const useMutateRevokeAgentWorkspaceAppLaunchSwr = (workspaceId: string) => useNivoMutation(["agentos", "workspace-app-launch-revoke", workspaceId], (launchId: string) => revokeAgentWorkspaceAppLaunch(launchId));
...
export const useMutateOrderAgentosSwr = () => useNivoMutation(["agentos", "catalog-order"], ({
  catalogItemSlug,
  catalogTierId
}: OrderAgentosCommand) => orderAgentOs(catalogItemSlug, catalogTierId), {
...
export const useMutateCreateAndPublishExpertSiteSwr = () => useNivoMutation(["apps", "expert-site-create-publish"], async (slug: string) => {
  ...
  invalidates: (_slug, answer) => answer.ok ? [expertSitesQueryKey, expertSiteDeploymentQueryKey(answer.data.id)] : [],
  shouldInvalidate: accepted
});

/** Execute one command against a module installation without sharing press state with neighbours. */
export const useMutateManageAgentosModuleRuntimeSwr = (installationId: string) => useNivoMutation(["agentos", "module-runtime", installationId], (input: ManageAgentosModuleRuntimeInput) => manageAgentosModuleRuntime(input));

/** Start one immutable module test run for the exact installation under test. */
export const useMutateRunAgentosModuleTestSwr = (installationId: string) => useNivoMutation(["agentos", "module-test", installationId], (input: RunAgentosModuleTestInput) => runAgentosModuleTest(input));

/** Apply one channel credential set to the exact AgentOS workspace. */
export const useMutateConfigureAgentWorkspaceChannelSwr = (workspaceId: string) => useNivoMutation(["agentos", "workspace-channel", workspaceId], (input: ConfigureAgentWorkspaceChannelInput) => configureAgentWorkspaceChannel(input), {
...
export const useMutateAgentosModuleAttachmentUploadSwr = (workspaceId: string, moduleId: string) => useNivoMutation(["agentos", "module-attachment-upload", workspaceId, moduleId], async ({
  file,
  mediaType
}: AgentosModuleAttachmentUploadCommand) => {
...
export const useMutateFinalizeAgentosModuleAttachmentSwr = (workspaceId: string, moduleId: string) => useNivoMutation(["agentos", "module-attachment-finalize", workspaceId, moduleId], (attachmentId: string) => finalizeAgentosModuleAttachment({
  agentWorkspaceId: workspaceId,
  moduleId,
  attachmentId
}), {
...
export const useMutateRemoveAgentosModuleAttachmentSwr = (workspaceId: string, moduleId: string) => useNivoMutation(["agentos", "module-attachment-remove", workspaceId, moduleId], (attachmentId: string) => removeAgentosModuleAttachment({
  agentWorkspaceId: workspaceId,
  moduleId,
  attachmentId
}), {
...
export const useMutateRunAgentosAiReadinessTestSwr = (workspaceId?: string) => useNivoMutation(workspaceId === undefined ? null : ["agentos", "ai-readiness-test", workspaceId], (idempotencyKey: string) => runAgentosAiReadinessTest({
...
export const useMutateReindexAgentWorkspaceKnowledgeSwr = (workspaceId: string) => useNivoMutation(["agentos", "ai-knowledge-reindex", workspaceId], (idempotencyKey: string) => reindexAgentWorkspaceKnowledge({
  workspaceId,
  idempotencyKey
}), {
