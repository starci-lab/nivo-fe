...
export const useMutateCreateWalletTopUpPayLinkSwr = () => useNivoMutation(["wallet-top-up-pay-link"], (input: WalletTopUpInput) => createWalletTopUpPayLink(input.amountVnd, input.returnUrl, input.cancelUrl));

/** Pay one invoice and refresh every account projection affected by settlement. */
export const useMutatePayInvoiceSwr = () => useNivoMutation(["invoice-pay"], payInvoice, {
