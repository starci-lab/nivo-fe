...
const integrationAnswer = (ok: boolean): AcademyIntegrationAnswer => ok ? {
  ok: true
} : {
  ok: false
};
const authorizationAnswer = (answer: Awaited<ReturnType<typeof beginAcademyZaloAuthorization>>): AcademyIntegrationAnswer => answer.ok ? {
  ok: true,
  authorizationUrl: answer.data.authorizationUrl
} : {
  ok: false
};
const signingAnswer = (answer: Awaited<ReturnType<typeof createAcademyWebhook>>): AcademyIntegrationAnswer => answer.ok ? {
  ok: true,
  signingSecret: answer.data.signingSecret
} : {
  ok: false
};
const executeAcademyIntegrationCommand = async (siteId: string, command: AcademyIntegrationCommand): Promise<AcademyIntegrationAnswer> => {
...
export const useMutateAcademyIntegrationSwr = (siteId: string) => useNivoMutation<AcademyIntegrationAnswer, AcademyIntegrationCommand>(["academy", "integration", siteId], command => executeAcademyIntegrationCommand(siteId, command), {
...
export const useMutateCreateAcademyStudentSwr = (siteId: string) => useNivoMutation(["academy", "student-create", siteId], createAcademyStudent, {
  invalidates: [["academy", "students", siteId]],
  shouldInvalidate: answer => answer.ok
});

/** Change one Academy student's status and refresh its collection and detail projections. */
export const useMutateSetAcademyStudentStatusSwr = (siteId: string, memberId?: string) => useNivoMutation(["academy", "student-status", siteId], setAcademyStudentStatus, {
  invalidates: memberId === undefined ? [["academy", "students", siteId]] : [["academy", "students", siteId], ["academy", "student", siteId, memberId]],
  shouldInvalidate: answer => answer.ok
});

/** Grant course access and refresh the affected student projection. */
export const useMutateGrantAcademyCourseAccessSwr = (siteId: string, memberId?: string) => useNivoMutation(["academy", "course-access-grant", siteId], grantAcademyCourseAccess, {
  invalidates: memberId === undefined ? [["academy", "students", siteId]] : [["academy", "students", siteId], ["academy", "student", siteId, memberId]],
  shouldInvalidate: answer => answer.ok
});

/** Revoke course access and refresh the affected student projection. */
export const useMutateRevokeAcademyCourseAccessSwr = (siteId: string, memberId?: string) => useNivoMutation(["academy", "course-access-revoke", siteId], revokeAcademyCourseAccess, {
  invalidates: memberId === undefined ? [["academy", "students", siteId]] : [["academy", "students", siteId], ["academy", "student", siteId, memberId]],
  shouldInvalidate: answer => answer.ok
});

/** Generate a reply draft without changing the durable lead collection. */
export const useMutateDraftLeadReplySwr = (siteId: string) => useNivoMutation(["academy", "lead-draft", siteId], draftLeadReply);

/** Advance or annotate a lead and refresh its owner-scoped collection. */
export const useMutateUpdateExpertSiteLeadSwr = (siteId: string) => useNivoMutation(["academy", "lead-update", siteId], updateExpertSiteLead, {
