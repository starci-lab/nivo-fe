...
import {
    ...
    myAgentosCustomModules,
    myAgentosModuleInstallation,
    myAgentosModuleInstallations,
    myAgentosModuleRuntime,
    myAgentosModuleTestRun,
    myAgentosModuleTestSurface,
...
export const useQueryMyAgentWorkspacesSwr = (enabled = true) => useNivoQuery(
    enabled ? agentWorkspacesQueryKey : null,
    myAgentWorkspace,
)
...
export const useQueryMyInvoicesSwr = (enabled = true) => useNivoQuery(
    enabled ? invoicesQueryKey : null,
    myInvoices,
)
/** Read the signed-in viewer's catalog orders when the consumer needs fulfillment data. */
export const useQueryMyCatalogOrdersSwr = (enabled = true) => useNivoQuery(
    enabled ? catalogOrdersQueryKey : null,
    myCatalogOrders,
)
/** Read one public catalog category behind a viewer-scoped cache key. */
export const useQueryCatalogItemsSwr = (category: CatalogCategory, enabled = true) => useNivoQuery(
    enabled ? ["catalog-items", category] : null,
...
export const useQueryMyAgentWorkspaceControlCenterSwr = (workspaceId: string, enabled = true) => useNivoQuery(
    enabled ? agentWorkspaceControlCenterQueryKey(workspaceId) : null,
    () => myAgentWorkspaceControlCenter(workspaceId),
)
/** Read the module-kind registry offered to the current viewer. */
export const useQueryMyAgentosSolutionModulesSwr = () => useNivoQuery(
    ["agentos", "solution-modules"],
    myAgentosSolutionModules,
)
/** Read all module installations in one workspace. */
export const useQueryMyAgentosModuleInstallationsSwr = (workspaceId: string) => useNivoQuery(
...
export const useQueryMyAgentosModuleInstallationSwr = (workspaceId: string, installationId: string) => useNivoQuery(
    ["agentos", "module-installation", workspaceId, installationId],
...
export const useQueryMyAgentosModuleRuntimeSwr = (
    ...
    ["agentos", "module-runtime", workspaceId, installationId, includeDiagnostics],
    () => myAgentosModuleRuntime(installationId, includeDiagnostics),
)
/** Read the immutable test surface for one module installation. */
export const useQueryMyAgentosModuleTestSurfaceSwr = (installationId: string, enabled = true) => useNivoQuery(
    enabled ? ["agentos", "module-test-surface", installationId] : null,
    () => myAgentosModuleTestSurface(installationId),
)
/** Read one durable module test run only after its identity exists. */
export const useQueryMyAgentosModuleTestRunSwr = (installationId: string, runId?: string) => useNivoQuery(
    runId === undefined ? null : ["agentos", "module-test-run", installationId, runId],
    () => myAgentosModuleTestRun(installationId, runId ?? ""),
)
/** Read one exact durable test run imperatively while a bounded poll is active. */
export const useReadMyAgentosModuleTestRun = (installationId: string) => useCallback(
    (runId: string) => myAgentosModuleTestRun(installationId, runId),
    [installationId],
)
/** Read custom modules visible in one AgentOS workspace. */
export const useQueryMyAgentosCustomModulesSwr = (workspaceId: string) => useNivoQuery(
    agentosCustomModulesQueryKey(workspaceId),
    () => myAgentosCustomModules(workspaceId),
)
/** Read the Studio projection for one custom module. */
export const useQueryMyAgentosCustomModuleStudioSwr = (workspaceId: string, moduleId: string) => useNivoQuery(
...
export const useQueryMyAgentosAiKnowledgeReadinessSwr = (workspaceId?: string, operationInFlight = false) => useNivoQuery(
    workspaceId === undefined ? null : agentosAiKnowledgeQueryKey(workspaceId),
    () => myAgentosAiKnowledgeReadiness(workspaceId ?? ""),
    {
        refreshInterval: (latest) => operationInFlight
            || latest?.ok === true && latest.data.readinessStatus === "testing"
            ? 2_000
            : 0,
    },
)
...
export const useQueryMyExpertSiteDeploymentSwr = (siteId?: string, polling = false) => useNivoQuery(
    siteId === undefined ? null : expertSiteDeploymentQueryKey(siteId),
    () => myExpertSiteDeployment(siteId ?? ""),
    { refreshInterval: polling ? 4_000 : 0 },
)
/** Read the Academy growth projection for one site. */
export const useQueryMyAcademyGrowthSnapshotSwr = (siteId: string) => useNivoQuery(
    ["academy", "growth", siteId],
    () => myAcademyGrowthSnapshot(siteId),
)
/** Read the Academy student collection for one site. */
export const useQueryMyAcademyStudentsSwr = (siteId: string) => useNivoQuery(
    ["academy", "students", siteId],
    () => myAcademyStudents({ siteId }),
)
/** Read one Academy student projection after a member is selected. */
export const useQueryMyAcademyStudentDetailSwr = (siteId: string, memberId?: string) => useNivoQuery(
    memberId === undefined ? null : ["academy", "student", siteId, memberId],
    () => myAcademyStudentDetail(siteId, memberId ?? ""),
)
/** Read configured Academy integrations for one site. */
export const useQueryMyAcademyIntegrationsSwr = (siteId: string) => useNivoQuery(
    ["academy", "integrations", siteId],
    () => myAcademyIntegrations(siteId),
)
/** Read captured leads for one expert site. */
export const useQueryMyExpertSiteLeadsSwr = (siteId: string) => useNivoQuery(
    ["academy", "leads", siteId],
    () => myExpertSiteLeads(siteId),
)
