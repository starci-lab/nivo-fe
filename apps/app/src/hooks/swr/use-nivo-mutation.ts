...
const invalidateQueries = async <TAnswer, TInput>(accessToken: string | null, input: TInput, answer: TAnswer, options: NivoMutationOptions<TAnswer, TInput> | undefined, mutateCache: ReturnType<typeof useSWRConfig>["mutate"]): Promise<void> => {
  if (accessToken === null || options?.shouldInvalidate?.(answer) === false) return;
  const invalidates = typeof options?.invalidates === "function" ? options.invalidates(input, answer) : options?.invalidates ?? [];
...
export const useNivoMutation = <TAnswer, TInput>(mutationKey: NivoMutationKey | null, mutation: (input: TInput) => Promise<TAnswer>, options?: NivoMutationOptions<TAnswer, TInput>) => {
  ...
  const key: NivoViewerMutationKey | null = typeof accessToken === "string" && accessToken.length > 0 && mutationKey !== null ? ["NIVO_MUTATION", viewerCacheKeyFor(accessToken), ...mutationKey] : null;
  return useSWRMutation(key, async (_key: NivoViewerMutationKey, {
    arg
  }: NivoMutationTrigger<TInput>) => {
