...
const decodeJwtSubject = (accessToken: string): string | null => {
    ...
    try {
        ...
        const decoded = JSON.parse(globalThis.atob(padded)) as { readonly sub?: unknown }
...
export const nivoViewerQueryKeyFor = (
    accessToken: string,
    queryKey: NivoQueryKey,
): NivoViewerQueryKey => ["NIVO_QUERY", viewerCacheKeyFor(accessToken), ...queryKey]
...
export const useNivoQuery = <TAnswer,>(
    ...
    const key: NivoViewerQueryKey | null = session.state.status === "signed-in"
