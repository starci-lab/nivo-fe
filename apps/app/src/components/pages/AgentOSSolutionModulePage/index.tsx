...
import {
    ...
    useQuerySupportCustomerMessagesSwr,
    useQuerySupportImportantFactsSwr,
    useQuerySupportTicketsSwr,
    useReadMyAgentosModuleTestRun,
    useMutateApproveSupportReplySwr,
    useMutateConfigureAgentWorkspaceChannelSwr,
    useMutateManageAgentosModuleRuntimeSwr,
    useMutateReconcileSupportDeliverySwr,
    useMutateRunAgentosModuleTestSwr,
    useMutateSetSupportTakeoverSwr,
} from "@/hooks/swr"
import {
    type AgentosModuleRuntime, type AgentosRuntimeValue, type ManageAgentosModuleRuntimeInput,
...
type AgentosModuleTestTarget = { readonly contextVersionId?: string; readonly setupSessionId?: string }
type OperationTarget = Extract<AgentOSSolutionModuleScreen, { readonly view: "operate" }>["contentProps"]["operationTarget"]
type SetupPane = Extract<AgentOSSolutionModuleScreen, { readonly view: "setup" }>["contentProps"]["compactPane"]
type TestPane = Extract<AgentOSSolutionModuleScreen, { readonly view: "test" }>["contentProps"]["compactPane"]
type DiagnosticsPane = Extract<AgentOSSolutionModuleScreen, { readonly view: "diagnostics" }>["contentProps"]["compactPane"]
type DiagnosticSignal = Extract<AgentOSSolutionModuleScreen, { readonly view: "diagnostics" }>["contentProps"]["selectedSignal"]
...
const SETUP_GATE_LABELS: Readonly<Record<string, string>> = {
    businessIdentity: "Business identity", productsServices: "Products and services", supportScope: "Support scope",
    customerSegments: "Customer segments", channels: "Channels", hoursAndSla: "Hours and SLA",
    escalationAndHandoff: "Escalation and handoff", prohibitedCommitments: "Prohibited commitments",
    privacyAndSensitiveData: "Privacy and sensitive data", toneAndLanguage: "Tone and language",
    automationPolicy: "Automation policy", readinessOwnership: "Readiness ownership", accountingScope: "Accounting scope",
    currencyAndLocale: "Currency and locale", sourceSystems: "Source systems", approvalPolicy: "Approval policy",
    approvalThresholds: "Approval thresholds", evidenceRequirements: "Evidence requirements", prohibitedActions: "Prohibited actions",
    schedulingScope: "Scheduling scope", timeZone: "Time zone", calendarSources: "Calendar sources",
    participantRules: "Participant rules", availabilityRules: "Availability rules", conflictPolicy: "Conflict policy",
    confirmationPolicy: "Confirmation policy", reminderPolicy: "Reminder policy", researchScope: "Research scope",
    sourcePolicy: "Source policy", citationPolicy: "Citation policy", confidencePolicy: "Confidence policy",
    prohibitedClaims: "Prohibited claims", freshnessPolicy: "Freshness policy",
}

const readableGate = (key: string): string => SETUP_GATE_LABELS[key]
...
const setupGatesFor = (
    ...
    const evidenceKeys = evidence.flatMap((candidate) => candidate !== null && typeof candidate === "object"
        && !Array.isArray(candidate) && typeof candidate.key === "string" ? [candidate.key] : [])
    return (fields.length > 0 ? fields : evidenceKeys).map((key) => {
        const row = evidence.find((candidate) => candidate !== null && typeof candidate === "object"
            && !Array.isArray(candidate) && candidate.key === key)
        return { key, label: readableGate(key), passed: row !== undefined && row.passed === true }
    })
}
...
const exactTestPassedFor = (testSurface: ReturnType<typeof exactTestSurfaceFor>, sessionId: string, digest: string | null): boolean => {
    ...
    return (testSurface?.runs ?? []).some((run) => (run.status === "passed" || run.status === "warning")
...
const contextDraftFor = (
    ...
    const summary = snapshot === null
        ? "Nivo is waiting for the owner to describe the business."
        : stringSetting(snapshot.summary, stringSetting(snapshot.businessIdentity, `Business context from Setup revision ${setup.setupRevision}`))
    return {
...
const activeVersionFor = (runtime: AgentosModuleRuntime): number | null => runtime.contextVersions.find(
    (context) => context.id === runtime.installation.activeContextVersionId,
)?.version ?? null
...
const executeSessionTitleFor = (title: string, index: number): string => (
    title === "New Execute session" ? `Conversation ${index + 1}` : title
)
...
export const AgentOSSolutionModulePage = ({ workspaceId, installationId, view = "setup" }: AgentOSSolutionModulePageProps) => {
    ...
    const supportMutationIdentity = { ...supportIdentity, conversationId: selectedSupportConversationId }
    ...
    useEffect(() => {
        ...
        if (selected !== undefined && (runtime.setupSession?.id === selected.id
            || selected.setupStatus === "open" || selected.setupStatus === "ready")) return
    ...
    const perform = useCallback(async (input: ManageAgentosModuleRuntimeInput): Promise<AgentosModuleRuntime | null> => {
        ...
        await runtimeQuery.mutate(result, { revalidate: false })
        return result.data
    ...
    const setSupportTakeover = useCallback((conversationId: string, takeover: boolean) => {
        void runSupportAction(() => takeoverMutation.trigger({ conversationId, takeover }))
    }, [runSupportAction, takeoverMutation])

    const reconcileSupportDelivery = useCallback((outboxId: string, delivered: boolean) => {
        void runSupportAction(() => deliveryMutation.trigger({ outboxId, delivered }))
    ...
    const runTest = useCallback(async (
        ...
        await testSurfaceQuery.mutate(result, { revalidate: false })
        ...
        for (let attempt = 0; attempt < POLL_ATTEMPTS; attempt += 1) {
            ...
            await testSurfaceQuery.mutate(next, { revalidate: false })
    ...
    const setupMessages: ReadonlyArray<SetupMessage> = selectedSetup === null ? [] : runtime.messages
    ...
    const setupRevisions: ReadonlyArray<SetupRevision> = runtime.setupSessions
        .filter((item): item is typeof item & { setupRevision: number; setupStatus: NonNullable<typeof item.setupStatus> } => (
            item.setupRevision !== null && item.setupStatus !== null
        ))
        .map((item) => ({ id: item.id, revision: item.setupRevision, status: item.setupStatus }))
    ...
    const executeMessages: ReadonlyArray<ExecuteMessage> = selectedSession === null ? [] : runtime.messages
        .filter((message) => message.sessionId === selectedSession.id)
        .map((message) => {
            const widget = widgetByMessage.get(message.id)
            const registration = widget === undefined ? undefined : runtime.installation.runtimeManifest.widgets.find(
                (candidate) => candidate.component === widget.rootComponent && candidate.version === widget.rootVersion,
            )
            ...
            return {
                ...
                widget: widget === undefined ? undefined : {
                    ...
                    node: task === undefined ? widget.tree : {
                        ...widget.tree,
                        props: { ...widget.tree.props, expectedVersion: task.expectedVersion },
                    },
    ...
    const operationTarget = selectedOperationTarget
        ?? (runtime.installation.kindKey === "customer-support" ? "customer-chat" : "internal-chat")
            ...
            contentProps: {
                ...
                onRun: (scenarioKey, scenarioInput) => {
                    if (draft !== null && draft.digest !== null) {
                        void runTest({ setupSessionId: draft.setupSessionId }, scenarioKey, scenarioInput)
                    }
                },
            },
        }
