...
import { _AcademyControlCenterPage, type AcademyControlCenterPageLabels } from "./component"
...
describe("_AcademyControlCenterPage", () => {
    it("renders restoring and refused notices", () => { expect(renderToStaticMarkup(<_AcademyControlCenterPage {...base} state="restoring" />)).toContain("Loading"); expect(renderToStaticMarkup(<_AcademyControlCenterPage {...base} state="refused" />)).toContain("Refused") })
    it("renders growth/system sections and optional public action", () => { const growth = renderToStaticMarkup(<_AcademyControlCenterPage {...base} state="ready" publicHost="academy.nivo.vn" />); expect(growth).toContain("growth-site-1"); expect(growth).toContain("students-site-1"); expect(growth).toContain("leads-site-1"); const system = renderToStaticMarkup(<_AcademyControlCenterPage {...base} state="ready" mode="system" />); expect(system).toContain("integrations-site-1") })
})
