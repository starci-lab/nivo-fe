...
const m = vi.hoisted(() => ({ locale: "vi", session: { state: { status: "signed-in", accessToken: "test-token" } }, leads: { ok: true, data: [] as unknown[] }, calls: { list: vi.fn(), update: vi.fn(), draft: vi.fn() } }))
...
beforeEach(() => { vi.clearAllMocks(); viewerSequence += 1; m.session.state.accessToken = `lead-pipeline-${viewerSequence}`; m.leads = { ok: true, data: [{ id: "lead-1", name: "Reader", contact: "reader@example.test", message: "Interested", status: "new", note: null }] }; m.calls.list.mockResolvedValue(m.leads); m.calls.update.mockResolvedValue({ ok: true }); m.calls.draft.mockResolvedValue({ ok: true, data: { reply: "Draft reply" } }) })
