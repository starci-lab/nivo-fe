...
describe("academy student CRM states", () => {
    it("renders list empty/refused states and student identity", () => {
        expect(renderToStaticMarkup(<_AcademyStudentCrm state="empty" students={[]} detailState="idle" labels={labels} on={handlers} />)).toContain("No students")
        expect(renderToStaticMarkup(<_AcademyStudentCrm state="refused" students={[]} detailState="idle" labels={labels} on={handlers} />)).toContain("Unavailable")
        expect(renderToStaticMarkup(<_AcademyStudentCrm state="answered" students={[student]} detailState="idle" labels={labels} on={handlers} />)).toContain("Reader")
    })
    ...
    it("renders detail progress and zero-course fallback", () => {
        ...
        const html = renderToStaticMarkup(<_AcademyStudentCrm state="answered" students={[student]} detailState="answered" detail={detail} labels={labels} on={handlers} />)
        expect(html).toContain("Intro")
        expect(html).toContain("2/4")
        const empty = renderToStaticMarkup(<_AcademyStudentCrm state="answered" students={[student]} detailState="answered" detail={{ ...detail, courses: [] }} labels={labels} on={handlers} />)
