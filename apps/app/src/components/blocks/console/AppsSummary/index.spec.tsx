...
import { AppsSummary } from "./index"

describe("AppsSummary", () => {
    it("draws owned application identity, lifecycle, and action without a total", () => {
        const html = renderToStaticMarkup(<AppsSummary label="Apps" state={{ phase: "populated", items: [{
    ...
    it("keeps a forbidden answer local to the section", () => {
        const html = renderToStaticMarkup(<AppsSummary label="Apps" state={{ phase: "forbidden", message: "Access denied" }} onOpenApp={vi.fn()} />)
