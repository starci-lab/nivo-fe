...
export type { OverviewPulseProps, OverviewPulseSignal } from "./component"

const STATUS_KEY: Readonly<Record<string, string | undefined>> = {
    not_provisioned: "status.notProvisioned", provisioning: "status.provisioning",
    awaiting_dns: "status.awaitingDns", ready: "status.ready", failed: "status.failed",
...
export const OverviewPulse = () => {
    ...
    const apps = (() => {
        ...
        return first === undefined
            ? { id: "apps", icon: "apps" as const, label: t("apps.title"), phase: "answered" as const, value: t("overview.none"), caption: t("apps.emptyDescription") }
            : { id: "apps", icon: "apps" as const, label: t("apps.title"), phase: "answered" as const, value: first.slug, caption: status(first.provisionStatus), emphasis: "accent" as const }
    })()
    const agent = (() => {
    ...
    const domains = (() => {
        ...
        return first === undefined
            ? { id: "domains", icon: "domains" as const, label: t("domains.title"), phase: "answered" as const, value: t("overview.none"), caption: t("domains.empty") }
            : { id: "domains", icon: "domains" as const, label: t("domains.title"), phase: "answered" as const, value: first.name, caption: domainCaption(first) }
    })()
    const wallet = (() => {
        ...
        return { id: "wallet", icon: "wallet" as const, label: t("wallet.title"), phase: "answered" as const, value: money(data.wallet.data.balanceVnd), caption: unpaid === undefined ? t("wallet.noUnpaid") : `${money(unpaid.amountVnd)} · ${t("wallet.dueAt", { date: day(unpaid.dueAt) })}`, emphasis: "accent" as const }
    })()
