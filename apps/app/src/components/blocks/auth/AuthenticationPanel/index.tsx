...
export type AuthState =
    /** The shortcut and the credential form. */
    | "details"
    /** The mailed code, and on the reset journey the new password beside it. */
    | "code"
    /** The journey finished. */
    | "done"
    /** The account holds a second factor this build cannot complete. Sign-in only. */
...
export type AuthenticationPanelProps =
    | (BlockProps<"details", AuthDetailsCopy> & { readonly on?: AuthActions })
    | (BlockProps<"code", AuthCodeCopy> & { readonly on?: AuthActions })
    | (BlockProps<"done", AuthNoticeCopy> & { readonly on?: AuthActions })
    | (BlockProps<"twoFactorUnsupported", AuthNoticeCopy> & { readonly on?: AuthActions })
...
const EMPTY = { email: "", password: "", confirmPassword: "", otp: "", newPassword: "" }
 ...
 */
export const AuthenticationPanel = (input: AuthenticationPanelProps) => {
     ...
     * gave `centred-title-pair` an optional glyph slot, and while `ChildrenOf` marks it optional the
     * caller still has to name it - omitting the key makes every `defineContractComponent` in this
     * file fail to resolve its overload, which reports as unrelated errors about `title`. Saying
     * `undefined` states the design decision out loud: this surface has no glyph above its name.
     */
    ...
    if (input.state === "done" || input.state === "twoFactorUnsupported") {
        return (
            <Tree
                ...
                render={defineContractComponent("centred-page-column", {
                    ...
                    body: [
                        defineContractComponent("centred-title-pair", {
                            ...
                            description: defineLeafComponent("text", { size: "sm" }, () => (
                                <Text props={{ content: input.props.doneHint, size: "sm", tone: "muted" }} />
                            )),
                        }),
                        defineContractComponent("stacked-peer-controls", {
                            control: [
                                ...
                                defineLeafComponent("button", {}, () => (
                                    <Button
                                        props={{ label: input.props.onwardLabel, variant: "primary" }}
                                        on={{ press: input.on?.onward }}
                                    />
                                )),
                            ],
                        }),
            ...
            <Tree
                ...
                render={defineContractComponent("centred-page-column", {
                    ...
                    body: [
                                            ...
                                            ...(!setsPassword ? [] : [
                                                defineCompositeComponent("field", {}, () => (
                                                    <Field
                                                        ...
                                                        on={{ change: (value) => { values.current.newPassword = value } }}
                                                    />
                                                )),
                                            ]),
                                            ...(status === undefined ? [] : [status]),
                        ...
                        /*
                         * BOTH ENDS FILLED, which is the contract's reason: asking for another code is
                         * the choice, abandoning the challenge is the way out of it.
                         *
                         * The resend keeps its `TextLink` while cooling down, with the label carrying
                         * the wait and the press withheld. `TextLink` has no `disabled` today, a `Text`
                         * is not admitted in that slot, and leaving it pressable sends a request the
                         * backend already refuses with `OTP_RESEND_TOO_SOON_EXCEPTION`.
                         */
                        defineContractComponent("spread-choice-row", {
                            choice: defineLeafComponent("text-link", { size: "sm" }, () => (
                                <TextLink
                                    props={{ label: isCoolingDown ? copy.cooldownLabel : copy.resendLabel, size: "sm" }}
                                    on={{ press: isCoolingDown ? undefined : input.on?.resend }}
                                />
                            )),
                            exit: defineLeafComponent("text-link", { size: "sm" }, () => (
                                <TextLink props={{ label: copy.backLabel, size: "sm" }} on={{ press: input.on?.back }} />
                            )),
                        }),
                    ],
                })}
            ...
            defineContractComponent("spread-choice-row", {
                choice: defineLeafComponent("checkbox", {}, () => (
                    <Checkbox
                        ...
                        on={{ change: (isRemembered) => input.on?.changeRememberMe?.(isRemembered) }}
                    />
                )),
                exit: defineLeafComponent("text-link", { size: "sm" }, () => (
                    <TextLink
                        props={{ label: copy.forgotPasswordLabel, size: "sm" }}
                    ...
                    defineContractComponent("auth-entry-stack", {
                        ...
                        credentials: defineLeafComponent("form", {}, () => (
                            <form onSubmit={submitDetails}>
                                <Tree
                                    contract="stacked-peer-controls"
                                    render={defineContractComponent("stacked-peer-controls", { control: credentialControls })}
                                />
                            </form>
                        )),
                    }),
                ],
                // A question and its answer read as one sentence, which is what makes the other
                // journey an alternative rather than a second offer of equal weight.
                footer: defineContractComponent("centred-prompt-row", {
                    prompt: defineLeafComponent("text", { size: "sm", tone: "muted" }, () => (
                        <Text props={{ content: copy.promptQuestion, size: "sm", tone: "muted" }} />
                    )),
                    action: defineLeafComponent("text-link", { size: "sm" }, () => (
                        <TextLink
                            props={{ label: copy.promptAction, size: "sm" }}
                            on={{ press: () => input.on?.changeMode?.(copy.mode === "signIn" ? "signUp" : "signIn") }}
                        />
                    )),
                }),
            })}
