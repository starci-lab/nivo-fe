...
export const AccountingWorkbenchBlockBase = (props: AccountingWorkbenchBlockProps) => {
  ...
  const primary = <FieldStack>
    ...
    <SurfaceCard label={t("documentIntake")}><form onSubmit={onIngest}><div className={ACCOUNTING_FORM_GRID_CLASS_NAME}>
      ...
      <Input id="accounting-document-amount" name="accounting-document-amount" label={t("documentAmount", { currency })} hint={t("currencyAmountHint")} value={documentAmount} onValueChange={setDocumentAmount} isRequired isDisabled={!intakeReady} isError={documentAmount.length > 0 && (documentAmountMinor === null || documentAmountMinor === "0")} errorMessage={documentAmount.length > 0 && (documentAmountMinor === null || documentAmountMinor === "0") ? t("positiveAmountRequired") : undefined} />
  ...
  const rail = <FieldStack>
    ...
    <SurfaceCard label={t("proposeCorrection")}><form onSubmit={onCorrection}><FieldStack>
      ...
      <Input id="accounting-delta" name="accounting-delta" label={t("correctionAmount", { currency })} hint={t("currencyAmountHintSigned")} value={deltaAmount} onValueChange={setDeltaAmount} isRequired isDisabled={isAsOf} isError={deltaAmount.length > 0 && (deltaAmountMinor === null || deltaAmountMinor === "0")} errorMessage={deltaAmount.length > 0 && (deltaAmountMinor === null || deltaAmountMinor === "0") ? t("nonZeroAmountRequired") : undefined} />
