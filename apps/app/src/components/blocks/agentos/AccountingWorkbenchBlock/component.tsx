...
export const AccountingWorkbenchBlockBase = (props: AccountingWorkbenchBlockProps) => {
  ...
  const documentRow = (document: AccountingDocument) => {
    ...
    return <Row key={document.id}>
      ...
      {action === null ? null : <Button size="sm" variant={action === "post" ? "primary" : "secondary"} isPending={pending} onPress={() => documentCommand(action, document.id)}>{t(documentActionKey(action))}</Button>}
  ...
  const correctionRow = (correction: AccountingCorrection) => {
    ...
    return <Row key={correction.id}>
      ...
      {access.approve ? <Button size="sm" variant="primary" isPending={approveCorrection.isMutating} onPress={() => approvePendingCorrection(correction.id)}>{t("approveCorrection")}</Button> : <Text size="xs" tone="muted" live="polite">{t(approvalReasonKey(access.approvalReason))}</Text>}
  ...
  const primary = <FieldStack>
    ...
    <SurfaceCard label={t("documentIntake")}><form onSubmit={onIngest}><FieldStack>
      <input ref={fileInput} type="file" accept="application/pdf,image/*,.pdf" hidden disabled={ingest.isMutating} aria-label={t("chooseEvidenceFile")} onChange={chooseFile} />
      <ActionRow><Button type="button" variant="secondary" isPending={ingest.isMutating} onPress={() => fileInput.current?.click()}>{fileName.length === 0 ? t("chooseEvidenceFile") : t("replaceEvidenceFile")}</Button><Text size="sm" tone="muted">{fileName.length === 0 ? t("noFileSelected") : t("selectedFile", { name: fileName, size: fileSize })}</Text></ActionRow>
      <label className={ACCOUNTING_NATIVE_FIELD_CLASS_NAME} htmlFor="accounting-classification"><Text size="sm" weight="semibold">{t("classification")}</Text><select className={ACCOUNTING_NATIVE_CONTROL_CLASS_NAME} id="accounting-classification" name="accounting-classification" value={classification} onChange={event => setClassification(event.currentTarget.value)} required><option value="income">{t("classificationIncome")}</option><option value="expense">{t("classificationExpense")}</option><option value="receivable">{t("classificationReceivable")}</option><option value="payable">{t("classificationPayable")}</option></select></label>
      <Input id="accounting-document-amount" name="accounting-document-amount" label={t("documentAmount", { currency })} hint={t("currencyAmountHint")} value={documentAmount} onValueChange={setDocumentAmount} isRequired isError={documentAmount.length > 0 && (documentAmountMinor === null || documentAmountMinor === "0")} errorMessage={t("positiveAmountRequired")} />
      <label className={ACCOUNTING_NATIVE_FIELD_CLASS_NAME} htmlFor="accounting-document-month"><Text size="sm" weight="semibold">{t("documentMonth")}</Text><input className={ACCOUNTING_NATIVE_CONTROL_CLASS_NAME} id="accounting-document-month" name="accounting-document-month" type="month" value={documentMonth} onChange={event => setDocumentMonth(event.currentTarget.value)} required /></label>
      <Button type="submit" variant="primary" isPending={ingest.isMutating} isDisabled={documentAmountMinor === null || documentAmountMinor === "0" || documentMonth.length === 0 || fileName.length === 0}>{t("addDocument")}</Button>
  ...
  const rail = <FieldStack>
    ...
    <SurfaceCard label={t("proposeCorrection")}><form onSubmit={onCorrection}><FieldStack>
      ...
      <Button type="submit" variant="primary" isPending={submitCorrection.isMutating} isDisabled={!correctionSubmitAllowed || !sourceEntryEligible || correctionReason.length === 0 || effectiveMonth.length === 0 || deltaAmountMinor === null || deltaAmountMinor === "0"}>{t("submitCorrection")}</Button>
    </FieldStack></form></SurfaceCard>
