...
export const AccountingWorkbenchBlockBase = (props: AccountingWorkbenchBlockProps) => {
  ...
  const rail = <FieldStack>
    ...
    <SurfaceCard label={t("proposeCorrection")}><form onSubmit={onCorrection}><FieldStack>
      ...
      <Button size="lg" type="button" variant="primary" isPending={submitCorrection.isMutating} isDisabled={!correctionSubmitAllowed || !sourceEntryEligible || correctionReason.length === 0 || effectiveMonth.length === 0 || deltaAmountMinor === null || deltaAmountMinor === "0"} onPress={submitCurrentCorrection}>{t("submitCorrection")}</Button>
