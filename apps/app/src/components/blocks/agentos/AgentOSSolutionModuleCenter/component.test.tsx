...
import { _AgentOSSolutionModuleCenter } from "./component"
...
describe("AgentOS solution module center", () => {
    it("renders refusal and empty installed states", () => {
        expect(renderToStaticMarkup(<_AgentOSSolutionModuleCenter {...base} state="refused" mode="catalog" cards={[]} />)).toContain("Unavailable")
        expect(renderToStaticMarkup(<_AgentOSSolutionModuleCenter {...base} state="answered" mode="installed" cards={[]} />)).toContain("No modules")
    })

    it("renders pending catalog cards and outcomes", () => {
        const html = renderToStaticMarkup(<_AgentOSSolutionModuleCenter {...base} state="answered" mode="catalog" cards={[card]} pendingId="sales" outcome="Started" />)
