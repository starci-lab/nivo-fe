...
export const AgentOSModuleSpecification = ({ workspaceId, moduleId }: AgentOSModuleSpecificationProps) => {
    ...
    const publish = async () => {
        ...
        try {
            const result = await publishModule.trigger({ acknowledgedVersion: version, idempotencyKey: `nivo-fe:${crypto.randomUUID()}` })
