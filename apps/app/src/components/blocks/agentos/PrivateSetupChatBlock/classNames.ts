...
/** Application spacing between the controlled draft row and its feedback lines. */
export const PRIVATE_SETUP_COMPOSER_CLASS_NAME = cn("flex", "min-w-0", "flex-col", "gap-3", "p-4");
/** The field and the action that submits it, side by side, so the send action stays in reach on a phone. */
export const PRIVATE_SETUP_COMPOSER_ROW_CLASS_NAME = cn("flex", "min-w-0", "items-end", "gap-3");
/** The app-owned wrapper that lets the message field span its column beside the send action. */
export const PRIVATE_SETUP_COMPOSER_FIELD_CLASS_NAME = cn("w-full", "min-w-0");
/** Spacing for immutable revision guidance. */
export const PRIVATE_SETUP_READONLY_CLASS_NAME = cn("flex", "min-w-0", "flex-col", "gap-2", "p-4");
/** Application width and a capped height around private Setup content: the conversation scrolls inside ChatWorkspace and the composer stays inside the frame. */
export const PRIVATE_SETUP_HOST_CLASS_NAME = cn("w-full", "min-w-0", "flex", "flex-col", "max-h-80");
