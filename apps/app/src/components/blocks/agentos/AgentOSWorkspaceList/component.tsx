import { ROW_CLASS_NAME, CONTENT_CLASS_NAME } from "./classNames";
...
export const AgentOSWorkspaceListBase = (props: AgentOSWorkspaceListProps) => {
    ...
    const collection = state === "empty" || state === "refused" ? <EmptyNotice message={props.props.message} actionLabel={state === "empty" ? props.props.actionLabel : undefined} onAction={state === "empty" ? props.on.create : undefined}/> :
        <DirectionList label={common.label} isLoading={loading}>{loading ? <div className={ROW_CLASS_NAME} data-contract="BOUNDARY-2 PADDING-4 PADDING-3"><Text isSkeleton>{common.label}</Text></div> : rows.map(row => <div key={row.id} className={ROW_CLASS_NAME} data-contract="BOUNDARY-2 PADDING-4 PADDING-3"><DirectionHeader level={2} title={<TextAction href={row.href} onFollow={props.state === "answered" ? () => props.on.openWorkspace(row.id) : undefined}>{row.name}</TextAction>} description={<><Text size="xs" tone="muted">{row.detail}</Text><Text size="xs" tone="muted">{row.kindLabel}</Text></>} action={<Badge tone={STATUS_TONE[row.status]}>{row.statusLabel}</Badge>}/></div>)}</DirectionList>;
    const summary = labels === undefined ? undefined : <SurfaceCard label={labels.workspaces}><div className={CONTENT_CLASS_NAME} data-contract="GAP-2"><Text size="sm" isSkeleton={loading}>{labels.workspaces}: {state === "refused" ? "—" : rows.length}</Text><Text size="sm" isSkeleton={loading}>{labels.running}: {state === "refused" ? "—" : rows.filter(row => row.status === "ready" || row.status === "active").length}</Text><Text size="sm" isSkeleton={loading}>{labels.attention}: {state === "refused" ? "—" : rows.filter(row => ["failed", "suspended", "awaiting_dns"].includes(row.status)).length}</Text></div></SurfaceCard>;
