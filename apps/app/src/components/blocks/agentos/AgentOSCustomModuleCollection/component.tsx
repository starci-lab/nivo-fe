...
export const AgentOSCustomModuleCollectionBase = ({ state, title, refused, empty, createLabel, rows, onOpen, onCreate }: AgentOSCustomModuleCollectionViewProps) => {
    ...
    const content = defineContractComponent("custom-module-collection", (input: LeafProps<SurfaceListCardData, SurfaceListCardActions>) => <Tree key={input.props.label} contract="custom-module-collection" render={defineContractComponent("custom-module-collection", { module: shown.map((row) => rowView(row, state === "loading", onOpen)) })} />)
