...
import { useQueryMyAgentosModuleInstallationSwr } from "@/hooks/swr"
...
export const AgentOSSolutionModuleDetail = ({ workspaceId, installationId }: AgentOSSolutionModuleDetailProps) => {
    ...
    const installation = query.data === undefined
        ? undefined
        : query.data.ok && query.data.data.agentWorkspaceId === workspaceId ? query.data.data : null
    ...
    return <AgentOSSolutionModuleDetailBase
        ...
        onOpenAiKnowledge={() => router.push(`/agentos/workspaces/${workspaceId}?view=ai-knowledge`)}
    />
}
