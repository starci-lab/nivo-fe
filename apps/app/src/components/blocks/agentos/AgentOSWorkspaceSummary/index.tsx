...
export const AgentOSWorkspaceSummary = (props: AgentOSWorkspaceSummaryProps) => {
    ...
    return <SurfaceCard label={labels.section}><div className="flex min-w-0 flex-col gap-2" data-contract="GAP-2"><>{fact(labels.status, data.workspace.status)}{fact(labels.plan, data.instance.planCode ?? "—")}{fact(labels.allocation, `${data.instance.ramMb} MB · ${data.instance.vcpu} vCPU`)}{fact(labels.host, data.instance.hostname)}{fact(labels.chart, data.instance.chartVersion)}</></div></SurfaceCard>;
};
