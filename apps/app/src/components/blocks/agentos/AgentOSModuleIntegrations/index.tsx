...
export const AgentOSModuleIntegrations = (props: AgentOSModuleIntegrationsProps) => {
  const {
    workspaceId,
    moduleId
  }: AgentOSModuleIntegrationsProps = props;
  ...
  const save = async () => {
    try {
      const result = await saveIntegration.trigger({
        providerKey: "helpdesk-api",
        secret
      });
      if (!result.ok) {
        setRefused(true);
  ...
  return <AgentOSModuleIntegrationsBase studio={studio ?? undefined} state={projectionState(refused, studio)} secret={secret} pending={saveIntegration.isMutating || removeIntegration.isMutating} labels={{
    title: t("title"),
    provider: t("provider"),
    field: t("field"),
