 ...
 */
export const Icon = (props: IconProps) => IconView(props);
const IconView = ({ props, isLoading = false }: IconProps) => {
  const glyph = GLYPHS[props.name];
  const role = props.role ?? "chip";
  const Glyph = glyph[role];
  if (isLoading) {
    return <span aria-hidden="true" className="size-5 shrink-0 animate-pulse rounded-full bg-default" />;
  }
