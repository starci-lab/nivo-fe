...
describe("SurfaceListCard loading action", () => {
    it("keeps the action slot visible while loading without an owner", () => {
        ...
        const surface = document.querySelector("[data-grammar-surface]")
        expect(surface).not.toHaveAttribute("data-grammar-state")
        expect(surface).not.toHaveAttribute("data-grammar-treatment")
        expect(surface?.querySelector("[data-grammar-list-mode='interactive']"))
            .toBeInTheDocument()
    })
})
